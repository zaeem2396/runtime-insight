...
final class RemediationBuilderConfigurationTest extends TestCase
{
    #[Test]
    public function it_suggests_env_and_config_checks(): void
    {
        $builder = new RemediationBuilder();
        $ctx = new RuntimeContext(
            exception: new ExceptionInfo('RuntimeException', 'env', 0, '/f.php', 1),
            stackTrace: new StackTraceInfo(frames: []),
            sourceContext: SourceContext::empty(),
        );
        $r = $builder->build(PrimaryCauseInferencer::CATEGORY_CONFIGURATION, $ctx);

        $joined = implode(' ', $r['fixes']);
        $this->assertStringContainsString('environment', strtolower($joined));
    }
}
